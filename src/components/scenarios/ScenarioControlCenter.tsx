...
import { Activity, TrendingUp, AlertTriangle, Users, Wrench, Zap } from 'lucide-react';
...
export const ScenarioControlCenter: React.FC<ScenarioControlCenterProps> = ({
  ...
  return (
    <div className="w-full space-y-4">
      {/* 6 Scenario Cards Grid with Action Sliders */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {SCENARIOS.map((scenario) => {
          ...
          return (
            <div
              key={scenario.id}
              className={`rounded-2xl p-5 text-left transition-colors flex flex-col justify-between select-none ${
                isActive
            ...
            >
              ...
              <div className="space-y-3 pt-3 border-t border-[#8ACBD0]">
                ...
                <div className="pt-1">
                  <ActionSlider
                    isActive={isActive}
                    label="SLIDE TO INJECT"
                    activeLabel="ACTIVE CONTINGENCY"
                    onActivate={() => onSelectScenario(scenario.id)}
                  />
                </div>
              </div>
            </div>
